...
class WorkflowTask(Base, UUIDMixin, TenantScopedMixin, AuditMixin):
    ...
    # Task details
    task_type = Column(String(50), nullable=False, index=True)  # Prepare, Review, Approve, File, Archive
    ...
    assigned_to_role_id = Column(UUID(as_uuid=True), ForeignKey("roles.id", ondelete="SET NULL"), nullable=True)

    # Status and dates
    status = Column(String(50), nullable=False, default="Pending", index=True)  # Pending, In Progress, Completed, Rejected
    ...
    assigned_user = relationship("User", foreign_keys=[assigned_to_user_id], back_populates="assigned_tasks")
    assigned_role = relationship("Role", foreign_keys=[assigned_to_role_id])

    # Self-referential for sub-tasks
    parent_task = relationship("WorkflowTask", remote_side="WorkflowTask.id", foreign_keys=[parent_task_id], backref="subtasks")

    # Indexes for common queries
    __table_args__ = (
        Index("idx_workflow_tasks_assigned_user_status", "assigned_to_user_id", "status", "due_date"),
